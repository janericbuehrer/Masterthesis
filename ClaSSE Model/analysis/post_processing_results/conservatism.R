...
setwd(path)
#events <- read_csv("output/VegF/m3/events.tsv")
...
#sampled_json <- sample_iterations(dat = json_data, n_samples = 3, burnin = FALSE)
...
## quantifying the number of conserved, relaxedly conserved and transition events
...
# barplot of event counts
barplot <- ggplot(summary_counters, aes(x = 1, y = mean_conservatism)) +
            geom_bar(stat = "identity", width = 0.5) +
            geom_errorbar(aes(ymin = mean_conservatism - sd_conservatism, ymax = mean_conservatism + sd_conservatism), width = 0.2) +
            geom_bar(aes(x = 2, y = mean_relaxed_conservatism), stat = "identity", width = 0.5) +
            geom_errorbar(aes(x = 2, ymin = mean_relaxed_conservatism - sd_relaxed_conservatism, ymax = mean_relaxed_conservatism + sd_relaxed_conservatism), width = 0.2) +
            geom_bar(aes(x = 3, y = mean_transition), stat = "identity", width = 0.5) +
            geom_errorbar(aes(x = 3, ymin = mean_transition - sd_transition, ymax = mean_transition + sd_transition), width = 0.2) +
            scale_x_continuous(breaks = c(1, 2, 3), labels = c("Conservatism", "Relaxed conservatism", "Transitions")) +
            labs(y = "Counts",  x = "", title = "Summary of event counts")+
            theme_minimal()
...
#### quantifying the specific biome boarder crossings ###
#
## summarise the number of conserved biomes per column (iteration)
#summary_state_conserved <- mySummary_state_conserved(results_state_conserved, vegf = TRUE)
#
## Rank-based ANOVA (Kruskal-Wallis Test) to test for significant differences between the states
#kruskal_test <-
#  summary_state_conserved %>% select(contains("mean_")) %>% kruskal.test(.)
...
##### into and out of plots ####
# example ta: crossing of treeline into and out from tundra and alpine regions
# pairwise comparison of the number of transitions into and out of the biome 
state_change_counts_into_ta <- state_change_counts_df %>%
  # Select columns containing "_ta" using regular expressions
  select(ends_with("_t")) %>%
  # Group by row and calculate the sum per row for selected columns
  rowSums()

state_change_counts_from_ta <- state_change_counts_df %>% 
  select(starts_with("t_")) %>%
  rowSums()
state_change_counts_alpine_crossings <- cbind("into"=state_change_counts_into_ta, "outOf"=state_change_counts_from_ta) %>% as.data.frame()
